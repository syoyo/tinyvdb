...
#else
    double total = 0.0;
    for (size_t i = 0; i < n; ++i) total += (double)a[i] * (double)b[i];
    return total;
#endif
}
...
static inline uint16_t tvdb__float_to_half_scalar(float v) {
    ...
    if (e <= 0) {
        // Subnormal or zero.
        if (e < -10) return s;  // underflow to zero
        m |= 0x800000u;
...
static inline void tvdb_simd_f16_to_f32(const uint16_t* in, float* out, size_t n) {
    ...
    for (; i < n; ++i) out[i] = tvdb__half_to_float_scalar(in[i]);
#else
    for (size_t i = 0; i < n; ++i) out[i] = tvdb__half_to_float_scalar(in[i]);
#endif
}
...
static inline void tvdb_simd_f32_to_f16(const float* in, uint16_t* out, size_t n) {
    ...
    for (; i < n; ++i) out[i] = tvdb__float_to_half_scalar(in[i]);
#else
    for (size_t i = 0; i < n; ++i) out[i] = tvdb__float_to_half_scalar(in[i]);
#endif
}
