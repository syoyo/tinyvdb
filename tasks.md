...
- [x] **Audit scalar writer paths for `INT32` / `INT64` / `DOUBLE` /
  ...
  voxel-by-voxel in `test_scalar_writer_audit`. Three bugs found and
  fixed: (a) typed builder set `descriptor.grid_type` to the template's
  string regardless of actual value type, so a DOUBLE grid was
  advertised as `Tree_float_5_4_3` and read back with 4-byte values;
  (b) reader's grid-type-string parser had no case for `int64_`, so
  any INT64 grid we wrote refused to reload; (c) BOOL grids stored
  voxels as 1 byte each on disk, not bit-packed — OpenVDB uses 1 bit
  per voxel for both leaf value buffers (`mWords[8]` / 64 bytes per
  8^3 leaf) and internal-node value buffers (`NodeMask<Log2Dim>`).
  Implemented a dedicated BOOL path in both leaf and internal-node
  reader/writer that bit-packs N voxels into N/8 bytes, bypassing
  the generic mask_values machinery (no flag byte, no
  inactive-value selection). On-disk format now matches OpenVDB's
  byte-for-byte at the leaf and internal-node value-buffer level.
  Cross-tool validated: `data/reference/ref_bool.vdb` (generated by
  libopenvdb 13.0 via `scripts/gen_openvdb_reference.cc`) round-trips
  through tinyvdb's reader → writer → re-reader, and the
  tinyvdb-written output is read back successfully by libopenvdb's
  `vdb_print` with matching active-voxel byte counts. Wired into
  `test_reference_roundtrip` as a permanent regression guard.
  Subsequent fix: BOOL **internal nodes** are not bit-packed — they
  go through OpenVDB's standard `readCompressedValues` path with one
  flag byte plus `num_values * sizeof(bool)` raw bytes. BOOL **leaves**
  additionally carry a 12-byte `mOrigin` between the value mask and
  the bit-packed buffer.
- [x] **Reader: integrate-and-test with delayed-load metadata grids.**
  No-op in practice: OpenVDB's `DelayedLoadMetadata` only affects the
  seek-skip path (`data == nullptr` in `readCompressedValues`), where
  it lets a stream skip per-leaf compressed buffers without reading
  them. tinyvdb always reads buffers (we never seek-skip), so the
  metadata is irrelevant on the load path. If the source file carries
  `file_delayed_load` as a grid metadata blob, our generic metadata
  reader passes it through as opaque bytes, which is also a no-op.
...
- [x] **`MultiPassIO` ≥ v224** handler. Multi-pass leaf-buffer I/O is
  only used by `PointDataGrid` in standard OpenVDB; tinyvdb already
  detects PointDataGrid (`is_point_data_grid` flag) and skips the
  buffer phase to `descriptor.end_byte_offset`, preserving the raw
  multi-pass payload as `point_data_blob`. No other built-in grid type
  requires multi-pass, so we treat the per-grid pass-count prefix as
  part of the opaque PointData blob rather than parsing it.
...
- [x] **Real `tvdb_nanovdb_get_voxel_f` / `_d`** (Root → Upper → Lower → Leaf
      traversal). Vendored `nanovdb::PNanoVDB.h` (Apache-2.0) under
      `src/third_party/nanovdb/` and dispatched the accessor through
      `pnanovdb_root_get_value_address` + `pnanovdb_read_float`/`_double`.
      Validated voxel-by-voxel against `nanovdb_convert`-produced
      `data/reference_nvdb/ref_float.nvdb` and `ref_double.nvdb`
      (64 voxels each, 0 errors).
- [x] **`tvdb_nanovdb_sample_trilinear_f`.** Cell-center convention,
      8-tap trilinear over the hierarchical accessor. Verified: sample
      at `(i+0.5, j+0.5, k+0.5)` returns `get_voxel_f(i,j,k)` exactly;
      midpoint between two voxels returns their mean.
...
- [x] **CRC32 checksum compute / validate.**
      ...
      Implements the polynomial 0xEDB88320 CRC32 plus the v32.6.0+
      `[GridData,TreeData]` head split and the 4KB blocked-CRC32 tail
      from `nanovdb::tools::GridChecksum`. Verified byte-exact against
      reference files: head matches `nanovdb_convert`'s default Half-mode
      checksum on `ref_float.nvdb`; both head and tail match the Full-mode
      checksum on `ref_float_full.nvdb` and the 2.4MB `sphere_full.nvdb`.
- [x] **Real BLOSC framing** (replaces fake LZ4-with-fake-header).
      Optional `TINYVDB_USE_SYSTEM_BLOSC=ON` CMake flag links system
      libblosc and routes `tvdb__nnvdb_decompress_blosc` through
      `blosc_decompress_ctx`. Verified on `ref_float_blosc.nvdb` (300KB
      → 1.8KB) and `sphere_blosc.nvdb` (2.4MB → 19KB) generated by
      `nanovdb_convert --blosc` — both produce identical voxel values
