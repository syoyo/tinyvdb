...
> Status: every P0 and P1 item below is implemented on both the Vulkan and
> CUDA/NVRTC backends with CPU-parity tests (28 cases in `test_gpu_backend`,
> all green; VRAM-bounded). Remaining "*Still open*" notes are optimizations
> (near-dense conv backend, unbounded hash-based construction) or an
> architectural ABI extension (cross-API external-memory import/export), not
> unimplemented compute ops. The P2 items are the tensor-framework features
> that tasks.md classifies as out of scope for the dependency-free C core.
