...
- [x] **P0: GPU TSDF integration and marching cubes.** `tvdb_gpu_integrate_tsdf`
      ...
      COMPLETE.
- [x] **P1: GPU grid construction.** `tvdb_gpu_points_to_mask` rasterizes a
      ...
      `tvdb_gpu_voxelize_points_unbounded` adds the hash-based path for point
      clouds whose ijk bbox volume exceeds VRAM: an open-addressing GPU hash set
      (CAS-claimed slots, linear probing, key-match dedup) sized O(point count)
      so memory is independent of the bbox volume, stream-compacted and finalized
      with an exact host-side dedup. `tvdb_gpu_voxelize_points` auto-falls-back to
...
- [x] **P1: GPU sampling/splatting gradients.** `tvdb_gpu_splat_trilinear_dense`
      ...
      the splat VJP w.r.t. values. `tvdb_gpu_sample_quadratic_dense_batch` adds
      the higher-order (triquadratic, 3x3x3 stencil / per-axis 3-point parabola)
      forward sampler on Vulkan + CUDA/NVRTC, mirroring
      `tvdb_sample_quadratic_dense`; covered by `test_gpu_backend`
      (`test_sample_quadratic`, incl. edge-clamped out-of-range points).
      `tvdb_gpu_splat_quadratic_dense` completes the pair as the 3x3x3-stencil
      scatter adjoint (the triquadratic-sample VJP w.r.t. the grid), with a new
      CPU reference `tvdb_splat_quadratic_dense` (zero-pad adjoint / skip
      out-of-range, matching the trilinear splat); covered by `test_gpu_backend`
      (`test_splat_quadratic`, 300-point scatter parity vs CPU on both backends).
...
- [x] **P2: Gaussian training helpers.** `tvdb_gpu_ssim` — windowed SSIM
      ...
      GPU-vs-CPU-reference parity + SSIM(x,x)=1). `tvdb_gpu_gaussian_sh_eval`
      adds view-dependent spherical-harmonics color evaluation (real SH, degree
      0–3 / K=(deg+1)² basis, 3DGS `max(Σ Yₖ·cₖ + 0.5, 0)` convention) on Vulkan
      + CUDA/NVRTC, with a new CPU reference `tvdb_gaussian_sh_eval`
      (tinyvdb_nanovdb.h); covered by `test_gpu_backend` (`test_gaussian_sh`:
      degree-3 parity vs CPU incl. a degenerate view direction).
      `tvdb_gpu_gaussian_project` adds the 3D→2D projection (mean through the
      camera, rotated covariance → 2D conic, screen-space radius) on Vulkan +
      CUDA/NVRTC, mirroring the CPU `tvdb_gaussian_project`; it reuses the CPU's
      fast-sqrt bit trick so the radius matches exactly, making the whole op
      parity-tight. Covered by `test_gpu_backend` (`test_gaussian_project`: 100
      Gaussians, all-fields parity vs CPU). Together with SH eval and the
      rasterizer this completes the Gaussian forward pipeline (project → SH →
      rasterize) on GPU. MCMC densification (Kheradmand et al. "3DGS as MCMC")
      is also done: `tvdb_gaussian_mcmc_relocation` recomputes per-copy opacity
      and scale (new_opacity = 1-(1-op)^(1/ratio); scale by op/denom over the
      binomial series in new_opacity) so splitting a Gaussian into `ratio` copies
      preserves the render, and `tvdb_gaussian_mcmc_add_noise` adds covariance-
      aware, opacity-gated exploration noise to the means from a caller-supplied
      standard-normal buffer (deterministic). Both have CPU references in
      `tinyvdb_nanovdb.h` and GPU `tvdb_gpu_gaussian_mcmc_relocation` /
      `_add_noise` (Vulkan + CUDA/NVRTC, one thread/Gaussian); covered by
      `test_gpu_backend` (`test_gaussian_mcmc`: relocation + noise parity vs CPU
      over 150 Gaussians, incl. the ratio==1 identity case). This closes the P2
      Gaussian-training-helpers item — the whole Gaussian subsystem is complete.
